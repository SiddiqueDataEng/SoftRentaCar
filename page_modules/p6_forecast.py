...
with tabs[0]:
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=[str(i) for i in hist.index], y=hist.values/1e6,
        name="Historical", line=dict(color=STEEL,width=2), mode="lines+markers", marker=dict(size=4)))
    fig.add_trace(go.Scatter(x=[str(i) for i in fc.index], y=fc.values/1e6,
    ...
    fig.add_trace(go.Scatter(x=x_ci, y=y_ci, fill="toself",
        fillcolor="rgba(230,57,70,.10)", line=dict(color="rgba(0,0,0,0)"), name="95% CI"))
    dark_layout(fig, "Monthly Revenue Forecast (PKR M)", xangle=-45, height=400)
    st.plotly_chart(fig, use_container_width=True)

    fc_df = pd.DataFrame({"Month":[str(i) for i in fc.index],
        "Forecast":fc.values.round(0), "Lower CI":lo.values.round(0), "Upper CI":hi.values.round(0)})
    for col in ["Forecast","Lower CI","Upper CI"]:
        fc_df[col] = fc_df[col].apply(lambda v: fmt(v))
...
with tabs[1]:
    ...
    cities = sorted(trips["pickup_city"].unique().tolist())
    sel_ci = st.selectbox("City", cities)
    days   = st.slider("Days Ahead", 7, 90, 30)

    with st.spinner("Forecasting …"):
        fc_dem = forecast_demand(gbm, le_c, feats_d, sel_ci, days)

    hist_d = trips[trips["pickup_city"]==sel_ci].copy()
    ...
    fig2 = go.Figure()
    fig2.add_trace(go.Scatter(x=rolling.index.astype(str), y=rolling.values,
        name="7-Day Rolling Avg", line=dict(color=STEEL,width=1.5)))
    fig2.add_trace(go.Scatter(x=fc_dem["date"].astype(str), y=fc_dem["predicted_trips"].round(1),
        name="Forecast", line=dict(color=BRAND,width=2.5,dash="dot"),
        mode="lines+markers", marker=dict(size=5,symbol="diamond"),
        fill="tozeroy", fillcolor="rgba(230,57,70,.08)"))
    dark_layout(fig2, f"Daily Demand Forecast — {sel_ci}", height=380)
    ...
    c = st.columns(3)
    kpi(c[0], f"{wd:.1f}", "Avg Weekday Demand", "trips/day")
    kpi(c[1], f"{we:.1f}", "Avg Weekend Demand", "trips/day")
    kpi(c[2], str(peak),   "Peak Demand Date",   "")
...
with tabs[2]:
    ...
    pp = st.columns(4)
    sel_mo  = pp[0].selectbox("Month",    list(months.keys()), format_func=lambda x:months[x], index=6)
    sel_dow = pp[1].selectbox("Day",      list(dows.keys()),   format_func=lambda x:dows[x])
    sel_cit = pp[2].selectbox("City",     sorted(trips["pickup_city"].unique().tolist()))
    sel_bt  = pp[3].selectbox("Booking",  sorted(trips["booking_type"].unique().tolist()))
    sel_dist= st.slider("Expected Distance (km)", 10, 1500, 150)

    if st.button("💡 Get Recommended Rate", type="primary"):
        rate = suggest_price(pm, le_pc, le_pt, feats_p, sel_mo, sel_dow, sel_cit, sel_bt, sel_dist)
        st.markdown(f"""
<div style="background:linear-gradient(135deg,#1e2a3a,#162030);border:2px solid {BRAND};
            border-radius:14px;padding:24px;text-align:center;margin:12px 0;">
  ...
  <div style="font-size:.72rem;color:#4a6a84;margin-top:3px;">
    {sel_cit} · {months[sel_mo]} · {dows[sel_dow]} · {sel_bt}
  </div>
    ...
    for city in cities6:
        row = []
        for m in range(1,13):
            try: p = suggest_price(pm, le_pc, le_pt, feats_p, m, 2, city, "City Ride", 50)
            except: p = 0
            row.append(p)
        surface.append(row)
    fig3 = go.Figure(go.Heatmap(z=surface, x=[months[m] for m in range(1,13)], y=cities6,
...
with tabs[3]:
    ...
    with col1:
        ...
        fig4 = go.Figure(go.Bar(x=yoy["invoice_year"].astype(str), y=yoy["total_amount_pkr"]/1e6,
            marker_color=BRAND, text=[f"{v:.1f}M" for v in yoy["total_amount_pkr"]/1e6],
            textposition="outside", textfont=dict(color=TEXT)))
        dark_layout(fig4, "Annual Revenue (PKR M)", height=320)
        st.plotly_chart(fig4, use_container_width=True)
    with col2:
        ty = trips.groupby("pickup_year").size().reset_index(name="n")
        fig5 = go.Figure(go.Scatter(x=ty["pickup_year"].astype(str), y=ty["n"],
            mode="lines+markers", line=dict(color=STEEL,width=2.5),
            fill="tozeroy", fillcolor="rgba(69,123,157,.15)", marker=dict(size=8)))
    ...
    sea["name"] = sea["_mo"].map(mn)
    fig6 = go.Figure(go.Bar(x=sea["name"], y=sea["n"],
