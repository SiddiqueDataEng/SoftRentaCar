...
# Build Sankey links — filter once so source/target/value stay aligned
valid_rows = mx[
    mx["pickup_city"].isin(top_cities) & mx["dropoff_city"].isin(top_cities)
].copy()

s_vals = [top_cities.index(r) for r in valid_rows["pickup_city"]]
t_vals = [len(top_cities) + top_cities.index(r) for r in valid_rows["dropoff_city"]]
v_vals = valid_rows["n"].tolist()
