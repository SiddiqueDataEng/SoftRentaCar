...
sel_drv = st.selectbox("Select Driver", sorted(driver_list), key="drv_select")
...
pp = st.columns(4)
harsh_b = pp[0].slider("Harsh Brakes",   0, 50, 5,   key="drv_hb")
harsh_a = pp[1].slider("Harsh Accels",   0, 50, 4,   key="drv_ha")
idle_m  = pp[2].slider("Idle Time (min)",0, 300, 60, key="drv_id")
speed_k = pp[3].slider("Speeding KM",    0, 200, 15, key="drv_sp")
pp2 = st.columns(4)
dist_km = pp2[0].slider("Distance KM",    1, 1500, 150,  key="drv_dk")
dur_hrs = pp2[1].slider("Duration (hrs)", 0.5, 48.0, 5.0,key="drv_dh")
max_spd = pp2[2].slider("Max Speed kmh",  40, 200, 110,  key="drv_ms")
avg_spd = pp2[3].slider("Avg Speed kmh",  5, 130, 55,    key="drv_as")

if st.button("🔍 Predict Profile", type="primary", key="drv_predict"):
